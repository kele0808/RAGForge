...
**Goal:** 从零做出一套最终能接近企业级 RAG 的系统。实施按步切片：文档进 PostgreSQL，先按可见性过滤再混合检索，生成带引用的答案。

**参考项目:** 每一步的方案和代码都对照 [infiniflow/ragflow](https://github.com/infiniflow/ragflow)（以及同类生产 RAG）。好的采纳，不好的按我们的方案。规则见 [`.cursor/rules/ragforge-learning-norms.mdc`](../../.cursor/rules/ragforge-learning-norms.mdc)。栈不同（Peewee+MySQL+ES+MinIO+Redis vs. SQLAlchemy+Postgres/pgvector）——是参考系，不是模板。
...
- 检索：先可见性谓词，再召回；过滤在 SQL 里，失败关闭；检索接口不调用 LLM
- 权限：资源是 Document；searcher 只调用 `visible_clause`。本步谓词可以是 `owner_id`，升级授权表只改这一处
- 节奏（全项目）：最终形态按企业 RAG 想；**每一步**只交付当前切片。方案和代码都要能升级，但不要提前铺下一片的实现
- 对照（全项目）：参考 RAGFlow 等现成实现；好的采纳，不好的按我们的方案，禁止照抄
...
| 6 | [06-acl.md](./06-acl.md) | 检索 SQL 带可见性谓词；别人的文档进不了召回 |
...
1. 只打开**当前步**的文档，不要提前把后面步骤的实现当作业。
2. 按文档里的文件列表创建文件；路径必须一致，后面的步骤会按这些名字引用。
3. 先写测试，跑红，再写实现，跑绿，再按文档里的 commit message 提交。
4. 「完成标准」全部勾上再进入下一步。
5. 写方案/代码时同时想：这一步的切片是否完整；下一片升级时是改一处还是改遍全库。
6. 对照 RAGFlow（或同类）的等价做法：好的采纳，不好的按我们的方案，并说清理由。
